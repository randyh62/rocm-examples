...
namespace reduction {
template <typename T, typename F> class v9 {
public:
  v9(const F &kernel_op_in, const T zero_elem_in,
            ...
            tmp::static_switch<std::array{32, 64}>(
                warp_size, [&]<int WarpSize>() noexcept {
                  tmp::static_switch<std::array{1, 2, 3, 4, 8, 16}>(
                      items_per_thread, [&]<int ItemsPerThread>() noexcept {
                        hipLaunchKernelGGL(
                            kernel<BlockSize, WarpSize, ItemsPerThread>,
                            dim3(new_size(factor, step_size)), dim3(BlockSize),
                            0, hipStreamDefault, front, back, kernel_op,
                            zero_elem, step_size);
                      });
                });
          });

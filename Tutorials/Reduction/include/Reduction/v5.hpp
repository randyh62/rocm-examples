...
namespace reduction {
template <typename T, typename F> class v5 {
public:
  v5(const F &kernel_op_in, const T zero_elem_in,
    ...
    auto kernel_dispatcher = [&](std::size_t step_size) {
      tmp::static_switch<std::array{32, 64}>(
          warp_size, [&]<int WarpSize>() noexcept {
            hipLaunchKernelGGL(
                kernel<WarpSize>, dim3(new_size(factor, step_size)),
                dim3(block_size), block_size * sizeof(T), hipStreamDefault,
...
private:
  ...
  __global__ static void kernel(T *front, T *back, F op, T zero_elem,
                                uint32_t front_size) {
